...
Table \ref{table:gqa_grounding} summarizes our results for grounding on the GQA dataset. The grounding score indicates whether the model attends to regions in the image that are relevant for the question. Hence, it shows how successful reasoning is grounded in an image. The MCAN model with BERT-base features performs best on grounding and significantly outperforms multi-task ViLBERT by over 30\%. This is especially interesting, as multi-task ViLBERT has consistently outperformed all MCAN models on the overall accuracy scores on VQA 2.0 and GQA. Comparing the best grounding score of ViLBERT, there is still a difference of over 10\% to the best scoring MCAN model.
...
We came to the conclusion that MCAN is better at relating textual and visual information, as it achieves a significantly higher grounding score compared to multi-task ViLBERT. % refer to attention visualization to support our claim?
% VQA is a very complex task, task specific model might be better to solve this task
Although multi-task ViLBERT scores a higher (overall) accuracy, its low grounding score reveals that it is not good at establishing meaningful/relevant relations between text and images. 
%despite its lower overall accuracy score. % ViLBERT has higher accuracy score, not as interesting/relevant to our research interest/question
% although ViLBERT has a bigger pool of information it can refer/relate to (due to pretraining) this is not enough to improve/support its grounding ability
We observed a slight increase of performance using contextual word representations/embeddings, by replacing GloVe word embeddings with BERT embeddings. This improvement could be attributed to BERT's contextual information, whereas GloVe is non-contextual.
To further exploit the bidirectional characteristic of BERT embeddings it might be interesting to feed them to a bidirectional LSTM. It might also be interesting to experiment with 

% change learning rate for BERT encoder
% explore contextual word representations derived from (possibly) better language model (e.g. RoBERTa)

%first briefly write what we did/approach to answer our research question: whether joint representations derived from pre-trained models are better compared to the joint representations learned from a task-specific model.
%answer research question: MCAN is better at relating textual and visual information, better grounding score, task specific, even though it gets a lower overall accuracy score
%go into more detail, comparison (what the models are good/bad at, relating to question)
%point to future work, directly related to improvements/benefit analysis/understanding of information flow 
%future work:
%datasets
% evaluate on VQA-CP (reference to \ref{subsection:vqa}) more insight on visual grounding
% evaluate on TDIUC (reference to \ref{subsection:vqa})
