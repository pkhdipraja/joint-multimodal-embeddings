...
class DataSet(BaseDataSet):
    def __init__(self, __C):
        ...
        # Tokenize
        self.token_to_ix, self.pretrained_emb, max_token = self.tokenize('openvqa/datasets/gqa/dicts.json', __C.USE_GLOVE, self.tokenizer)
        self.token_size = self.token_to_ix.__len__()
        print(' ========== Question token vocab size:', self.token_size)

        self.max_token = -1
        if self.max_token == -1:
            self.max_token = max_token + 2 if self.__C.USE_BERT else max_token
    ...
    def tokenize(self, json_file, use_glove, tokenizer):
    ...
    def proc_ques(self, ques, token_to_ix, max_token, tokenizer):
        if tokenizer is not None:
            words = re.sub(
                r"([.,'!?\"()*#:;])",
                '',
            ...
            ques_ix = torch.squeeze(ques_ix).numpy()
        else:
