...
def test_engine(__C, dataset, state_dict=None, validation=False):
    ...
    for step, (
        ...
        if __C.DATASET in ['gqa']:
            att = img_att.cpu().data.numpy()
            att_list.append(att)
    ...
    if __C.DATASET in ['gqa']:
        att_weights = np.squeeze(np.concatenate(att_list))
    ...
    if __C.DATASET in ['gqa']:
        EvalLoader(__C).eval(dataset, ans_ix_list, pred_list, att_weights, result_eval_file, ensemble_file, log_file, validation)
    else:
